...
public class Win1123H2Strategy(ChooserOptions options): Win11Strategy(options) {
    ...
    public override bool submitChoice(AutomationElement desiredChoice, IReadOnlyList<AutomationElement> authenticatorChoices, bool isShiftDown) {
        if (isLocalWindowsHelloTpmPrompt) {
            return false;
        } else if (options.fidoEl.FindFirst(TreeScope.Children, NEXT_BUTTON_CONDITION) is not { } nextButton) {
            LOGGER.Error("Could not find Next button in Windows Security dialog box, skipping this dialog box instance");
            return false;
        } else if (!base.submitChoice(desiredChoice, authenticatorChoices, isShiftDown)) {
            return false;
        } else {
